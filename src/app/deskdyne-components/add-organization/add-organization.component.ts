...
export class AddOrganizationComponent implements OnInit {
  ...
  constructor(private apiMainService: ApiMainService, 
    private googleMapService: GoogleMapService,private chgDetRef:ChangeDetectorRef, private modalService: NgbModal, private runtimeStorageService: RuntimeStorageService, private router: Router, private fb: FormBuilder) {
  ...
  patchFormValue(org: any) {
    ...
    org.cafeteriaList.forEach(async (cafe:any,i:any)=>{
      if(cafe.clusterId){
        ...
        let clusterDetailsnew=this.runtimeStorageService.getCacheData('CLUSTERS_details');
        clusterdetails=clusterDetailsnew;
        }
      }
      this.add_cafeteria();
      this.form.controls['cafeteriaList'].at(i).controls['cafeteria_location'].patchValue({
      ...
      this.form.controls['cafeteriaList'].at(i).controls['poc_details'].patchValue({
      ...
      this.form.controls['cafeteriaList'].at(i).controls['poc_details'].controls['approverDetails'].patchValue({
    ...
    org.poc_details.forEach((el: any, i: any) => {
      ...
      this.form.controls['poc_details'].at(i).controls['approverDetails'].patchValue({
  ...
  toggleMap(index:any){
    ...
    this.modalService.open(this.geolocation, { ariaLabelledBy: 'modal-basic-title', size: 'lg', windowClass: 'mapModel' })
    .result.then((result) => {
      if (result === 'add') {
  ...
  async patchCafeLocation(){
    this.form.controls['cafeteriaList'].at(this.cafeteriaIndex).controls['address2'].patchValue(this.cafeLocation.location);
    this.form.controls['cafeteriaList'].at(this.cafeteriaIndex).controls['cafeteria_location'].controls['lat'].patchValue(this.cafeLocation.latlng.lat)
    this.form.controls['cafeteriaList'].at(this.cafeteriaIndex).controls['cafeteria_location'].controls['lng'].patchValue(this.cafeLocation.latlng.lng)
    let cluster = await this.googleMapService.getClusterName(this.cafeLocation.latlng);
    let clusterDetails=this.runtimeStorageService.getCacheData('CLUSTERS_details');
    console.log(clusterDetails,"cluster");
    this.form.controls['cafeteriaList'].at(this.cafeteriaIndex).controls['clusterId'].patchValue(clusterDetails.clusterId)
    this.form.controls['cafeteriaList'].at(this.cafeteriaIndex).controls['clusterName'].patchValue(clusterDetails.clusterName)
  }
  ...
  patchSelectedAdmins(index: any) {
    ...
    console.log(this.form.controls['poc_details'].at(index).controls)
    this.form.controls['poc_details'].at(index).controls['approverDetails'].patchValue({
  ...
  patchCafeAdmins(index: any) {
    this.form.controls['cafeteriaList'].at(index).controls['poc_details'].patchValue({
    ...
    this.form.controls['cafeteriaList'].at(index).controls['poc_details'].controls['approverDetails'].patchValue({
  ...
  async updateOrg() {
    try {
      ...
      await this.apiMainService.B2B_org_update(this.form.getRawValue(), this.viewOrg._id);
      this.clearRunTimeStorage();
      this.router.navigate(['B2B_search_org']);
    } catch (error) {
      console.log(error)
