...
export class ChecklistQuestionComponent {
  ...
  async getAllChecklistQuestions() {
    try {
      const allChecklistQuestions = await this.apiMainService.getAllChecklistQuestions();
  ...
  showPopup(QuestionObj: any) {
    this.confirmationModalService.modal(`Are you sure, you want to delete ${QuestionObj.checklistQuestion}`,
      () => this.deletechecklistQuestion(QuestionObj._id), this);
  }
}
