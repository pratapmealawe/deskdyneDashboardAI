...
export class EmployeeIndividualSnackMenuComponent implements OnInit, OnChanges {
  ...
  async getEmployeeIndividualSnackMenuByCafeteria(): Promise<void> {
    try {
      ...
      const menuItems: SnackMenuMeta = await this.api.getEmployeeIndividualSnackMenu(this.selectedCafeteria.cafeteria_id);
      if (menuItems) {
  ...
  async editBulkMenu(): Promise<void> {
    ...
    try {
      const res = await this.api.saveEmployeeIndividualSnackMenu(bulkMenuObj);
      if (res) {
