...
export class CakeMenuComponent implements OnInit {
  ...
  async copyOrgMenu() {
    try {
      if (this.orgSelected) {
        const menuItems = await this.ddApiMainService.b2b_fetchBulkCakeMenu(this.orgSelected);
  ...
  async getBulkMenuItems() {
    try {
      ...
      const menuItems = await this.ddApiMainService.b2b_fetchBulkCakeMenu(this.orgObj._id);
      console.log(menuItems)
      this.bulkMenuFetched = menuItems;
      if (this.bulkMenuFetched) {
        this.bulkMenuList = menuItems.itemList ? menuItems.itemList : [];
  ...
  async editBulkMenu() {
    ...
    try {
      ...
      await this.ddApiMainService.b2b_updateBulkCakeMenu(bulkMenuObj);
      this.editMode = false;
