...
export class OrgBulkSnackboxMenuComponent {
  @Input() orgObj: any;
  ...
  async copyOrgMenu(){
    try {
      if(this.orgSelected){
        const menuItems = await this.ddApiMainService.B2B_fetchBulkSnacksMenu(this.orgSelected);
        console.log(menuItems)
        this.bulkSnacksMenuFetched = menuItems;
        if (this.bulkSnacksMenuFetched) {
  ...
  async getBulkSnackMenuItems() {
    try {
      console.log(this.orgObj._id)
      const menuItems = await this.ddApiMainService.B2B_fetchBulkSnacksMenu(this.orgObj._id);
      this.bulkSnacksMenuFetched = menuItems;
      console.log(menuItems)
      if (this.bulkSnacksMenuFetched) {
        this.bulkMenuList = menuItems.itemList ? menuItems.itemList : [];
  ...
  async editBulkMenu() {
    ...
    try {
      ...
      await this.ddApiMainService.B2B_Bulk_SnackMenuAdd(bulkMenuObj);
      this.editMode = false;
