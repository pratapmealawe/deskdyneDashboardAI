...
export class OrgBulkMenuComponent implements OnInit {
  ...
  async getBulkMenuItems() {
    try {
      ...
      const menuItems = await this.ddApiMainService.B2B_fetchBulkMenu(this.orgObj._id);
      console.log(menuItems)
      this.bulkMenuFetched = menuItems;
      if (this.bulkMenuFetched) {
        this.bulkMenuList = menuItems.itemList ? menuItems.itemList : [];
  ...
  async editBulkMenu() {
    ...
    try {
      ...
      await this.ddApiMainService.B2B_saveBulkMenu(bulkMenuObj);
      this.editMode = false;
