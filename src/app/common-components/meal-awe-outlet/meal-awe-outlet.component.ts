...
export class MealAweOutletComponent implements OnInit {
  ...
  constructor(private apiMainService: ApiMainService, private modalService: NgbModal, private router: Router) {
  ...
  async fetchMealAweOutlet(id: any) {
    try {
      ...
      if (mealAweOutlet && mealAweOutlet._id) {
        ...
        this.mealAweOutletObj.showOnlyToEmployees = mealAweOutlet.showOnlyToEmployees;
  ...
  async getFooditemList() {
    try {
      ...
      mealPackageList = [...mealPackageList].map(ele => { ele.selected = false; return ele; });
  ...
  openMealPackageList() {
    if(this.showUpdate == false){
      this.mealPackageList = [...this.mealPackageList].map(ele => { ele.selected = false; return ele;})
    }
    this.modalService.open(this.content, {ariaLabelledBy: 'modal-basic-title', size: 'xl',windowClass: 'mapModel'})
    .result.then((result) => {
  ...
  async updateOutlet(outlet: any) {
    try {
      const updatedOutlet = this.apiMainService.updateMealAweOutlet(outlet._id, outlet);
  ...
  async enableDisableOutlet() {
    if (this.mealAweOutletObj._id) {
      ...
      try {
        await this.apiMainService.setOutletOpenedStatus(this.mealAweOutletObj._id, status);
