...
@Component({
  selector: 'app-employee-individual-meal-menu',
  standalone: true,
...
export class EmployeeIndividualMealMenuComponent implements OnInit, OnChanges {
  ...
  async getEmployeeIndividualMenuByCafeteria(): Promise<void> {
    ...
    try {
      ...
      const menuItems: IndMenuMeta = await this.api.getEmployeeIndividualMealMenu(this.selectedCafeteria.cafeteria_id);
      if (menuItems) {
