...
export class SweetMenuComponent implements OnInit, OnChanges {
  @Input() orgObj!: Org;
  ...
  async getBulkMenuItemsByCafeteriaId(): Promise<void> {
    ...
    try {
      const menuItems: SweetMenuMeta = await this.api.B2B_fetchSweetMenu(this.selectedCafeteria.cafeteria_id);
      if (menuItems) {
