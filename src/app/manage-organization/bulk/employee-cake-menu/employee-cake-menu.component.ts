...
export class EmployeeCakeMenuComponent implements OnInit, OnChanges {
  ...
  async getEmployeeCakeMenuByCafeteria(): Promise<void> {
    try {
      ...
      const menuItems: CakeMenuMeta = await this.api.getEmployeeCakeMenu(this.selectedCafeteria.cafeteria_id);
      if (menuItems) {
  ...
  async editBulkMenu(): Promise<void> {
    ...
    try {
      const res = await this.api.saveEmployeeCakeMenu(bulkMenuObj);
      if (res) {
