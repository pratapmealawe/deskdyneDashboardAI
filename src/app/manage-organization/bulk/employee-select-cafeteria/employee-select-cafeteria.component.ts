...
import { OrganizationAddVendorComponent } from 'src/app/manage-organization/bulk/organization-add-vendor/organization-add-vendor.component';
import { OrganizationCopyBulkMenuComponent } from 'src/app/manage-organization/bulk/organization-copy-bulk-menu/organization-copy-bulk-menu.component';
...
@Component({
  selector: 'app-employee-select-cafeteria',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MaterialModule],
  templateUrl: './employee-select-cafeteria.component.html',
  styleUrls: ['./employee-select-cafeteria.component.scss']
})
export class EmployeeSelectCafeteriaComponent implements OnInit, OnChanges {
  ...
  openCategoryModal(): void {
    if (this.toggleCategory) {
      this.modalRef = this.modalService.open(this.toggleCategory, {
        centered: true,
        backdrop: 'static'
      });
    }
  ...
  copyMenu(): void {
    ...
    const dialogRef = this.dialog.open(OrganizationCopyBulkMenuComponent, {
      width: '800px',
      data: payload,
      autoFocus: true,
      disableClose: false
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.toaster.success('Menu copied successfully');
      }
    });
  }
