...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MaterialModule, FormsModule, ReactiveFormsModule, HighchartsChartModule, OutletOrderCardComponent]
})
export class CustomerOutletOrdersComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userSub = this.customerSharedService.userDetails$.subscribe(details => {
      if (details) {
  ...
  setDateRange(type: 'today' | 'yesterday' | 'last7days' | 'last30days' | 'thisMonth') {
    const now = new Date();
    let start = new Date();
    let end = new Date();
    ...
    this.dateForm.patchValue({ dateFrom: start, dateTo: end });
    this.applyDateRange();
  }

  isRangeActive(type: string): boolean {
    ...
    return normalize(dateFrom) === normalize(s) && normalize(dateTo) === normalize(e);
  }
  ...
  async getOrderListByCustomerId() {
    ...
    try {
      ...
      const orderList: any = await this.apiMainService.getOutletOrdersByCustomerId(this.userDetails._id, dateFrom, dateTo);
  ...
  hasActiveFilters(): boolean {
    return !!(this.filterOrderStatus || this.filterPgName || this.filterAppVersion || this.filterPlatform || this.filterIsPosOrder);
  }
  ...
  applyFilters() {
    ...
    if (this.filterOrderStatus) {
      list = list.filter(o => o.orderstatus === this.filterOrderStatus);
    }
