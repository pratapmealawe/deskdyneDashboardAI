...
export class AddOrganizationComponent implements OnInit {
  ...
  checkboxOptions = [
    ...
    { key: 'showVendingMachine', label: 'Show Vending Machine' },
  ...
  async updateOrglevelSubsidy() {
    try {
      ...
      let res = await this.apiMainService.updateOrglevelSubsidy(
        this.orgSubsidy,
        this.viewOrg._id
      );
      if (res && res._id) {
  ...
  async updateCafelevelSubsidy(index: any) {
    try {
      ...
      let res = await this.apiMainService.updateCafelevelSubsidy(
        subsidy,
        cafeteria_Id
      );
      console.log(res);
