...
export class ImportEmployeeComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: { orgObj: any, selectedCafeterias?: any }
  ) { }
  ...
  async downloadTemplate() {
    try {
      ...
      saveAs(blob, `Employee_Import_Template_${this.data.orgObj?.organization_name || 'Org'}.xlsx`);
        ...
        data.forEach((elm: any) => {
          ...
          if (elm[0] && elm[0] !== 'Emp Name' && elm[0] !== 'Employee Name') {
            excelEmployees.push({
              ...
              // Map to first selected cafeteria if any
              cafeteria_list: this.data.selectedCafeterias || []
            });
          }
        });
