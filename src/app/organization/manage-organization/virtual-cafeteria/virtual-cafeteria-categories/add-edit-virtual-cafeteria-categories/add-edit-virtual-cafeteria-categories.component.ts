...
@Component({
  selector: 'app-add-edit-virtual-cafeteria-categories',
  standalone: true,
  imports: [CommonModule, MaterialModule, ReactiveFormsModule, DragDropModule, ImageCropperComponent],
  templateUrl: './add-edit-virtual-cafeteria-categories.component.html',
  styleUrls: ['./add-edit-virtual-cafeteria-categories.component.scss']
})
export class AddEditVirtualCafeteriaCategoriesComponent {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<AddEditVirtualCafeteriaCategoriesComponent>,
    ...
    reader.onload = () => {
      ...
      dialogRef.afterClosed().subscribe((result: any) => {
        if (result && result.confirm && result.croppedImages) {
          this.categoryImgFile = result.croppedImages.file;
          this.categoryForm.patchValue({ categoryImg: result.croppedImages.resizeDataUrl });
        }
      });
    };
  ...
  async sendCategoryToServer() {
    ...
    try {
      ...
      if (isEdit && categoryId) {
        response = await this.apiMainService.updateVirtualCafeteriaCategory(cafeteria_id, fd);
      } else {
        response = await this.apiMainService.addVirtualCafeteriaCategory(fd);
      }
