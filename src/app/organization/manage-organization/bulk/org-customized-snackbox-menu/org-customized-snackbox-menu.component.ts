...
export class OrgCustomizedSnackboxMenuComponent implements OnInit, OnChanges {
  ...
  async getCustomizedSnackBoxMenuItemsByCafeteriaId(): Promise<void> {
    try {
      ...
      const menuItems: SnackMenuMeta = await this.api.B2B_fetchCustomizedFoodBoxMenu(this.selectedCafeteria.cafeteria_id);
      if (menuItems) {
  ...
  async editBulkMenu(): Promise<void> {
    ...
    try {
      const res = await this.api.B2B_saveCustomizedFoodBoxMenu(bulkMenuObj);
      if (res) {
