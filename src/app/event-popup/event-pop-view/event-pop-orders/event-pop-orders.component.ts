...
@Component({
  ...
  templateUrl: './event-pop-orders.component.html',
  styleUrls: ['./event-pop-orders.component.scss'],
  standalone: true,
  imports: [
    CommonModule,
    MaterialModule,
...
export class EventPopOrdersComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (!dateFrom || !dateTo || !this.eventObj?._id) {
      this.dateForm.markAllAsTouched();
      return;
    }
    this.fetchOutletOrders({ outletId: this.eventObj._id, fromDate: dateFrom, toDate: dateTo });
  }
  ...
  generateChartData() {
    ...
    this.chartOptions = {
      ...
      title: { text: 'Orders by Date and Status', style: { color: '#1e3a8a', fontWeight: 'bold' } },
      xAxis: { categories: categories, labels: { style: { color: '#64748b' } } },
      yAxis: { allowDecimals: false, min: 0, title: { text: 'Number of Orders' }, gridLineDashStyle: 'Dash' },
  ...
  async excelExport() {
    ...
    const worksheet = workbook.addWorksheet('Event Pop Orders');
    ...
    saveAs(blob, `event_pop_orders_${this.eventObj?.eventPopupName || 'event'}_${new Date().toISOString().slice(0, 10)}.xlsx`);
  }
  ...
  downloadPdf() {
    ...
    const docDefinition: any = {
      ...
      content: [
        { text: 'Event Pop Orders Report', style: 'header' },
        { text: `Event: ${this.eventObj?.eventPopupName || '-'}`, style: 'subheader' },
    ...
    (pdfMake as any).createPdf(docDefinition).download(`EventOrders_${dateStr}.pdf`);
  }
}
