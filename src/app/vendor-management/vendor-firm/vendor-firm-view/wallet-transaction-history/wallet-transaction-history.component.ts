import { Component, Input, OnChanges, OnInit, OnDestroy, SimpleChanges, Optional } from '@angular/core';
...
export class WalletTransactionHistoryComponent implements OnChanges, OnInit, OnDestroy {
    ...
    constructor(
        ...
        @Optional() private vendorFirmViewService: VendorFirmViewService
    ) { }
    ...
    ngOnInit(): void {
        // Only subscribe if we are in the main view (not a dialog with explicit input)
        if (!this.isDialog && this.vendorFirmViewService) {
    ...
    private buildQueryBody() {
        ...
        const body: any = {
            vendorFirmId: this.vendorFirmInfo.vendorFirmId || this.vendorFirmInfo._id,
